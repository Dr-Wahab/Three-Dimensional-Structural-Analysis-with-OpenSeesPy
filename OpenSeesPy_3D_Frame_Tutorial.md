...
```
```bash
python make_figures.py              # every figure in this tutorial
```

`make_figures.py` writes PNG files to `./figures/`. All figures in this document were
produced by that script — none is a sketch, and each can be regenerated after any change
...
Running `opsv.plot_model()` and `opsv.plot_extruded_shapes_3d()` on the finished model gives
the wireframe and the true section outlines:

![Model geometry and node numbering](figures/fig01_model.png)

*Figure 1 — Wireframe model with node tags. Base nodes 1–4, then 101–104, 201–204, 301–304.
The numbering scheme of §7 is directly readable off the plot.*

![Extruded sections](figures/fig02_extruded.png)

*Figure 2 — The same model with sections extruded to true size. This view is the fastest way
to catch a beam that has been laid on its side: the 30 × 60 beams must appear deeper than
they are wide, and the 45 × 45 columns square. Compare with §10.2.*
...
![Deformed shape under gravity](figures/fig03_gravity_defo.png)

*Figure 3 — Deformed shape under 1.2D + 1.6L, magnified 300×. The beams sag in double
curvature and the joints rotate slightly; the columns shorten axially by 0.413 mm. Note the
absence of sway — a symmetric structure under symmetric load must deform symmetrically, and
this plot is the visual form of verification check §20.2.*

![Bending moment My under gravity](figures/fig04_gravity_My.png)

*Figure 4 — Bending moment M<sub>y</sub> under gravity. Hogging at the beam ends, sagging at
mid-span, with the columns picking up the unbalanced joint moment.*

The three-dimensional diagram is good for seeing the pattern but poor for reading values. For
a single member, plot the moment directly from the end forces and the closed-form parabola:

![Beam bending moment diagram](figures/fig04b_beam_bmd.png)

*Figure 5 — Beam 1–2 at level 1. The end moment of 60.26 kN·m sits below the fully fixed
value of 68.80 kN·m (dashed), the difference being the rotational flexibility of the columns.
The identity M<sub>end</sub> + M<sub>mid</sub> = wL²/8 closes to the third decimal — this is
verification check §20.3 drawn rather than tabulated.*

![Axial force](figures/fig04c_gravity_N.png)

*Figure 6 — Axial force N. The columns accumulate load downward — roughly 119 kN per storey —
while the beams carry almost no axial force, since the rigid diaphragm takes in-plane forces
and the load is symmetric.*

![Shear force](figures/fig04d_gravity_Vz.png)

*Figure 7 — Shear force V<sub>z</sub>. Each beam shows the linear variation from +wL/2 to
−wL/2 expected of a uniformly loaded member, crossing zero at mid-span exactly where the
moment in Figure 5 reaches its maximum.*
...
![Mode 1](figures/fig05_mode1.png)

*Figure 8 — Mode 1, pure translation in X. All floors move in phase, amplitude increasing
with height.*

![Mode 2](figures/fig05_mode2.png)

*Figure 9 — Mode 2, pure translation in Y.*

![Mode 3](figures/fig05_mode3.png)

*Figure 10 — Mode 3, torsion about the vertical axis. The floors rotate in plan. This mode
exists in the model **only because** rotational mass inertia J<sub>m</sub> was assigned at the
diaphragm master nodes (§12.3). Omit it and this mode disappears entirely.*
...
![Modal summary](figures/fig06_modal_summary.png)

*Figure 11 — Left: modal periods, with the ASCE 7-22 empirical period T<sub>a</sub> marked.
Right: participating mass by mode and direction. The strict one-direction-per-mode pattern is
the signature of a doubly symmetric plan; introduce an eccentricity (Exercise E3) and the
bars begin to share modes.*
...
![ELF profiles](figures/fig07_elf_profiles.png)

*Figure 12 — The four plots that summarise an ELF analysis. Left to right: the lateral force
distribution (linear in height because k = 1.0); the storey shear staircase, accumulating
downward to the base shear of 99.4 kN; the lateral displacement profile; and the drift check
against the 2 % limit. Note how much margin the frame has — the design drift bar is roughly
one sixth of the limit.*

![Deformed shape under ELF](figures/fig08_elf_defo.png)

*Figure 13 — Deformed shape under the lateral forces, magnified 300×. The columns bend in
double curvature with a point of contraflexure near mid-height, which is the classic
shear-frame mechanism and the reason the portal formula of §20.6 works as well as it does.*
...
### 18.2 The ground motion: what OpenSees needs
...
### 18.3 Generating an artificial, spectrum-compatible wave

Real records are the first choice when suitable ones exist. Often they do not — the site has
no strong-motion history, or the code requires the input to be compatible with a specific
design spectrum. The standard answer is an **artificial accelerogram**: a synthetic signal
whose 5 %-damped response spectrum matches a target within a stated tolerance.

Note carefully what "artificial" does *not* mean. It is not simply random noise. Filtered
noise has an entirely arbitrary spectral shape, so it excites the structure by accident.
A spectrum-matched wave delivers, at every period, exactly the demand the design spectrum
prescribes.

**The target.** ASCE 7-22 §11.4.6 defines the design response spectrum from two parameters,
S<sub>DS</sub> and S<sub>D1</sub>:

$$S_a =
\begin{cases}
S_{DS}\left(0.4 + 0.6\,T/T_0\right) & T < T_0\\[2pt]
S_{DS} & T_0 \le T \le T_s\\[2pt]
S_{D1}/T & T_s < T \le T_L\\[2pt]
S_{D1}T_L/T^{2} & T > T_L
\end{cases}
\qquad T_s = \frac{S_{D1}}{S_{DS}},\quad T_0 = 0.2\,T_s$$

We adopt S<sub>DS</sub> = 1.00 g and S<sub>D1</sub> = 0.60 g, giving T<sub>s</sub> = 0.60 s
and T<sub>0</sub> = 0.12 s. Our T₁ = 0.316 s sits on the constant-acceleration plateau, so the
target demand at the fundamental period is S<sub>a</sub> = 1.00 g. The zero-period value is
0.4 S<sub>DS</sub> = 0.40 g, which is the PGA the record should end up with.

**The matching algorithm.** The classical frequency-domain scheme (Gasparini & Vanmarcke) is
four steps, iterated:

1. Start from Gaussian white noise multiplied by an **intensity envelope**, so the signal is
   non-stationary like a real earthquake — a parabolic rise, a strong-motion plateau, and an
   exponential decay.
2. Compute the 5 %-damped response spectrum S<sub>a</sub><sup>(k)</sup>(T) of the current
   signal.
3. Scale the **Fourier amplitude** at each frequency by the ratio
   S<sub>a</sub><sup>target</sup>/S<sub>a</sub><sup>(k)</sup>, evaluated at T = 1/f, and leave
   the **phase untouched**. Phase is what carries the non-stationary character; destroy it and
   you get a signal that is spectrally correct but looks nothing like an earthquake.
4. Re-apply the envelope and repeat.
...
for it in range(niter):
    ...
    a = np.fft.irfft(np.fft.rfft(a) * scale * hp, n)
    if it < niter - 3:
        a *= env
```

Two details in that loop are not cosmetic, and both were found by inspecting the result
rather than by reasoning in advance:

- **`left=1.0, right=1.0`.** Outside the matched period band the ratio is undefined. If you
  extrapolate the nearest value instead of clamping to unity, the scheme amplifies
  unconstrained long-period content on every single iteration. In our first attempt this drove
  the peak ground displacement to 1.9 m and the PGV/PGA ratio to 0.28 s — roughly double any
  recorded value. Clamping fixed it.
- **The high-pass taper `hp`.** A cosine taper removing content below 0.20 Hz and passing
  fully above 0.40 Hz. Without it the integrated ground displacement drifts without bound,
  because a spectrum match places no constraint on the near-DC part of the signal.

**Computing the spectrum efficiently.** Step 2 must be repeated ~30 times, so a naive
period-by-period loop is too slow. The `response_spectrum` function uses the
**Nigam–Jennings** recurrence, which is *exact* for a piecewise-linear excitation, and
vectorises it over the period vector — one loop over time serves all 120 periods at once:

$$u_{i+1} = A u_i + B\dot{u}_i + C p_i + D p_{i+1}, \qquad
\dot{u}_{i+1} = A' u_i + B'\dot{u}_i + C' p_i + D' p_{i+1}$$

with the eight coefficients depending only on ω<sub>n</sub>, ζ and Δt.

**The result.**

![Artificial ground motion](figures/fig10_artificial_motion.png)

*Figure 14 — The artificial accelerogram, with the velocity and displacement obtained by
integration. PGA = 0.439 g against the target zero-period value of 0.40 g; PGV = 0.648 m/s;
PGV/PGA = 0.15 s, which sits in the range typical of recorded firm-soil motions. The envelope
shape is visible: build-up over the first 2 s, strong motion to about 14 s, then decay. The
residual acceleration after 22 s is 0.8 % of the peak.*

![Spectral match](figures/fig11_spectral_match.png)

*Figure 15 — Left: the matched spectrum against the ASCE 7-22 target, with a run of unmatched
filtered noise shown for contrast — note how erratically the unmatched signal wanders around
the target, over-exciting some periods and under-exciting others. Centre: the match ratio.
Over 0.10–2.00 s the mean absolute misfit is 2.98 % and **no period falls below 90 % of
target**, which is the acceptance criterion of ASCE 7-22 Ch. 16. Right: the Fourier amplitude
spectrum, showing where the energy actually sits relative to the structure's f₁ = 3.16 Hz.*

The match degrades below T = 0.10 s, where the ratio falls to about 0.87. That band is
outside the range of engineering interest here — the highest mode carrying meaningful mass
has T = 0.054 s and contributes 3 % — but it should be reported rather than hidden. Matching
at very short periods is intrinsically hard because there is little Fourier energy there to
scale.

### 18.4 Newmark integration
...
### 18.5 The stepping loop

```python
nodes = [mid(k) for k in range(1, NS + 1)]
...
Three points. Subtracting `u0` reports displacement **relative to the gravity-deformed
state**, which is what you want. The return value of `analyze` is checked every step, so a
divergence is caught where it happens rather than discovered later in a nonsensical plot. And
recording *every* floor rather than only the roof costs nothing and gives you the drift
envelope for free — `ops.reactions()` inside the loop likewise buys the base-shear history.

### 18.6 Results: response histories

```
[5] LINEAR TIME-HISTORY ANALYSIS
  Record: artificial, matched to the ASCE 7-22 design spectrum (SDS = 1.00 g, SD1 = 0.60 g)
  n = 3000;  dt = 0.0100 s;  duration = 30.0 s
  PGA = 0.439 g   PGV = 0.648 m/s   PGD = 0.713 m
  Spectral match over 0.10-2.00 s: mean |misfit| = 2.98 %,  ratio range 0.915 - 1.065
  Sa at T1 = 0.3160 s : record 0.969 g  vs target 1.000 g
  Rayleigh: alphaM = 1.03043 1/s, betaK = 0.002423 s  (zeta = 5 % at modes 1-2)
```

![Response history](figures/fig12_response_history.png)

*Figure 16 — Top: displacement of all three floors. The three traces are almost perfectly in
phase and their ratio is constant — the response is dominated by the first mode, exactly as
the 84.6 % participating mass predicted. Bottom: base shear, with the ELF design shear drawn
for scale. Both quantities decay after about 15 s at the rate set by 5 % Rayleigh damping.*

### 18.7 Response envelopes, and why they dwarf the ELF results

Taking the maximum of the absolute response at each level over the whole record gives the
envelope:

| Level | u<sub>max</sub> (mm) | drift<sub>max</sub> | ELF drift | ratio |
|---|---|---|---|---|
| 1 | 9.644 | 0.00357 | 0.00043 | 8.32 |
| 2 | 22.471 | 0.00477 | 0.00058 | 8.21 |
| 3 | 30.710 | 0.00305 | 0.00038 | 8.02 |

| Quantity | Value |
|---|---|
| Peak roof displacement | 30.71 mm = H/264 |
| Peak base shear | 833.24 kN = 0.839 W |
| ELF design base shear | 99.35 kN = 0.100 W |
| **Ratio** | **8.39** |

![Response envelopes](figures/fig13_response_envelopes.png)

*Figure 17 — Left: displacement envelopes. Centre: drift envelopes, both still far inside the
2 % limit. Right: four estimates of base shear.*

**That factor of 8.4 is not an error, and understanding it is the point of this section.**

The ELF base shear is a *reduced design* force. The coefficient C<sub>s</sub> = 0.10 already
contains the division by the response modification factor R = 8, because codes expect the
structure to respond inelastically and dissipate energy through ductile behaviour. Our
time-history analysis, by contrast, uses **linear elastic elements that never yield**. It
therefore reports the force an elastic structure would actually attract, which is roughly
R times the design force. The measured ratio of 8.39 against R = 8.0 is precisely what the
code's logic implies.

The right-hand panel of Figure 17 makes this concrete with four numbers that should — and do —
line up:

| Estimate | Value | Basis |
|---|---|---|
| ELF design | 99 kN | C<sub>s</sub>W, reduced by R |
| ELF × R | 795 kN | un-reducing the design force |
| Time history | 833 kN | full 3D transient analysis |
| S<sub>a</sub>(T₁)·W·M₁* | 814 kN | first mode and the record's own spectrum |

Three independent elastic estimates agreeing within 5 % is a strong verification of the whole
dynamic chain — mass, stiffness, damping, record scaling and integration.

The same logic explains the drift comparison, and exposes something worth knowing. The code
amplifies elastic design drift by C<sub>d</sub> = 5.5, not by R = 8. Since the true elastic
demand is R times the design force, the code's estimate of real displacement is deliberately
lower than a fully elastic calculation, by a factor of R/C<sub>d</sub> = 8/5.5 = 1.45. That is
not an inconsistency in the code; it reflects the empirical observation that a yielding
structure's peak displacement is somewhat less than the equal-displacement rule would give at
these period ranges. But it does mean **an elastic time-history analysis of a structure
designed for ductility will always show drifts larger than the code check** — and if you do
not know why, you will think one of the two analyses is wrong.

> **The practical lesson.** Never compare a linear time-history result directly with a
> code-reduced design force. Either un-reduce the design force by R, or replace the elastic
> elements with nonlinear ones that can actually yield. Exercise E9 does the former; E8 sets
> up the latter.

Section 20.9 verifies the 30.71 mm peak against an independent response-spectrum calculation.
...
![Overturning](figures/fig09_overturning.png)

*Figure 18 — Left: how the applied overturning moment of 607.7 kN·m is resisted — 70 % by the
axial couple between windward and leeward columns, 30 % by column base moments. Right: the
resulting axial redistribution. Columns 1 and 4 (windward, x = 0) are relieved to 323.8 kN and
columns 2 and 3 (leeward) loaded to 389.5 kN, symmetric about the gravity-only value of
356.6 kN. This is verification check §20.5 in graphical form.*
...
Γ₁·φ₁,roof = **1.2850**. Computing the 5 %-damped spectral displacement of the *same*
artificial record at T₁ = 0.3160 s by independent SDOF Newmark integration in NumPy gives
S<sub>d</sub> = **24.138 mm** (S<sub>a</sub> = 0.973 g — reassuringly close to the 1.000 g
target the record was matched to).

$$u_{\text{roof}}^{\max} \approx 1.2850 \times 24.138 = 31.02~\text{mm}$$

The full time-history analysis gave **30.710 mm** — a difference of **1.0 %**.
...
![Spectrum check](figures/fig14_spectrum_check.png)

*Figure 19 — Left: the first mode shape against the normalised response envelope from the
time history. They lie almost on top of one another, which is the visual statement that the
response is first-mode dominated and therefore that a single-mode prediction should work.
Right: the two independent predictions of peak roof displacement.*
...
| 9 | Time history vs spectrum | 30.71 vs 31.02 mm, 1.0 % |
| 10 | Elastic base shear three ways | 795 / 833 / 814 kN, within 5 % |
...
**11. Comparing an elastic time-history result with a code-reduced design force.**
Not a modelling error but an interpretation error, and a serious one. It leads people either
to conclude their model is broken (a factor of eight is alarming) or, worse, to "correct" a
perfectly good model until the numbers agree.
*Detect it:* check whether the ratio is close to R. If it is, nothing is wrong — see §18.7.
...
The result, produced by `make_figures.py`, looks like this — use it to check your own:

![Period vs height](figures/fig15_period_vs_height.png)

*Figure 20 — Computed fundamental period against height for NS = 1 to 10, compared with the
ASCE 7-22 empirical period and its C<sub>u</sub>T<sub>a</sub> upper bound. The computed curve
tracks T<sub>a</sub> closely at low rise and drifts toward the upper bound as the frame gets
taller and more flexible — remaining inside the code band throughout. That the band is not
crossed over a ten-fold range of height is a strong global validation of the model's
mass–stiffness balance. Ask yourself why the computed curve is steeper than the empirical one,
and what would happen to it if you enabled `CRACKED = True`.*
...
**E9 — Un-reducing the design force.** Re-run the ELF analysis with C<sub>s</sub> = 0.839
(the elastic demand measured in §18.7 instead of the reduced 0.10) and compare the resulting
drifts with the time-history envelope. They should agree closely. What does that tell you
about the accuracy of the ELF *distribution* as opposed to its *magnitude*?

**E10 — Record-to-record variability.** Generate five artificial records by changing only the
`seed` argument, run each, and tabulate the peak roof displacement. Compute the mean and
coefficient of variation. This is why codes require a *suite* of records rather than one, and
why the mean of at least seven records may be used instead of the maximum.

**E11 — Match quality versus response.** Deliberately degrade the match by reducing `niter`
to 3, and re-run. How much does the peak response change, and does it change in the direction
you expected from the match ratio at T₁?
...
[5] LINEAR TIME-HISTORY ANALYSIS
  ...
  Level    u_max [mm]   drift_max    ELF drift    THA/ELF
     1        9.644     0.00357      0.00043      8.32
     2       22.471     0.00477      0.00058      8.21
     3       30.710     0.00305      0.00038      8.02
  Peak roof displacement = 30.710 mm = H/264
  Peak base shear        = 833.24 kN = 0.8387 W
  ELF base shear         = 99.35 kN = 0.1000 W
  ratio THA / ELF        = 8.39   (compare R = 8.0: the ELF forces are already reduced by R,
                                the time history is fully elastic)
  Check: Sa(T1)*W*M1* = 814.16 kN vs THA peak 833.24 kN  (2.3 %)
...
Running `python make_figures.py` writes fifteen PNG files to `./figures/`:

| File | Content | Section |
|---|---|---|
| `fig01_model.png` | Wireframe model with node tags | §5.1 |
| `fig02_extruded.png` | Extruded sections | §5.1 |
| `fig03_gravity_defo.png` | Deformed shape, gravity | §15.2 |
| `fig04_gravity_My.png` | Moment diagram M<sub>y</sub>, 3D | §15.2 |
| `fig04b_beam_bmd.png` | Single-beam BMD with hand check | §15.2, §20.3 |
| `fig04c_gravity_N.png` | Axial force diagram | §15.2 |
| `fig04d_gravity_Vz.png` | Shear force diagram | §15.2 |
| `fig05_mode1/2/3.png` | Mode shapes 1–3 | §16.2 |
| `fig06_modal_summary.png` | Periods and participating mass | §16.3 |
| `fig07_elf_profiles.png` | Force, shear, displacement, drift | §17.3 |
| `fig08_elf_defo.png` | Deformed shape, ELF | §17.3 |
| `fig09_overturning.png` | Overturning resistance breakdown | §19.2, §20.5 |
| `fig10_artificial_motion.png` | Artificial wave: a, v, d | §18.3 |
| `fig11_spectral_match.png` | Target vs matched spectrum, FFT | §18.3 |
| `fig12_response_history.png` | Floor displacements, base shear | §18.6 |
| `fig13_response_envelopes.png` | THA vs ELF envelopes | §18.7 |
| `fig14_spectrum_check.png` | Spectrum vs time history | §20.9 |
| `fig15_period_vs_height.png` | Parametric study | §22 (E1) |
...
- `make_figures.py` — every figure in this document, regenerated from the model
